...
import { FaCar, FaTrash, FaSpinner, FaEdit, FaSearch, FaPlus } from "react-icons/fa";
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-6 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          ...
          <div className="flex-1 md:mx-4 relative">
            <input
              type="text"
              placeholder="Rechercher par marque, modèle ou utilisateur..."
              className="pl-10 pr-4 py-3 w-full border-2 border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-lg"
              value={searchTerm}
          ...
          <button
            onClick={() => navigate('/addCarAnnouncement')}
            className="bg-blue-600 text-white px-6 py-3 rounded-lg flex items-center space-x-2 hover:bg-blue-700 transition-colors text-lg font-medium shadow-lg"
          >
            ...
            <table className="min-w-full divide-y divide-gray-200">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {filteredCars.length > 0 ? (
                  filteredCars.map(car => (
                    <tr key={car._id} className="hover:bg-gray-50">
                      <td className="px-6 py-4 whitespace-nowrap flex items-center">
                        {car.cars_images ? <img src={car.cars_images} alt={car.marque} className="h-12 w-12 object-cover rounded mr-2"/> : <FaCar className="text-gray-500 mr-2"/>}
                        {car.marque}
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">{car.model}</td>
                      <td className="px-6 py-4 whitespace-nowrap">{car.year}</td>
                      <td className="px-6 py-4 whitespace-nowrap">{car.username || '-'}</td>
                      <td className="px-6 py-4 whitespace-nowrap flex space-x-2">
                        <button onClick={() => openEditModal(car)} title="Modifier" className="text-blue-600 hover:text-blue-800"><FaEdit/></button>
                        <button onClick={() => deleteCar(car._id)} disabled={deletingId===car._id} title="Supprimer" className={`${deletingId===car._id?'opacity-50 cursor-not-allowed':'text-red-600 hover:text-red-800'}`}>
                          {deletingId===car._id? <FaSpinner className="animate-spin"/>:<FaTrash/>}
                        </button>
                      </td>
                    </tr>
                  ))
                  ...
                  <tr>
                    <td colSpan="5" className="text-center py-4 text-gray-500">Aucune voiture trouvée</td>
                  </tr>
                )}
              </tbody>
            </table>
